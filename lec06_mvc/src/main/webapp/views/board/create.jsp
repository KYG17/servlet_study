...
<body>
	...
	<section>
		<div id="section_wrap">
			...
			<!-- enctype="multipart/form-data" input type="file" 이 있을 때 form태그 안에 무조건 써야함 -->
				...
				<%-- 	<input type="hidden" name="board_writer" value="<%=m.getMemberNo()%>"> --%>
					...
					<input type="file" name="board_file" accept=".png,.jpg,.jepg"><br>
	...
	<script type="text/javascript">
		const createBoardForm = function() {
			...
			if(!form.board_title.value){
				...
				/* 확장자는 .jpg 이런식이니깐 .을 기준으로 index를 찾음 */
				const idx = val.lastIndexOf('.');
				const type = val.substring(idx+1,val.length);
				if(type == 'jpg' || type == 'png' || type == 'jpeg'){
					//form.submit();
					//console.log(val);
					//파일 데이터를 보내줘야 하는 속성들 cache , async , contentType , processType
